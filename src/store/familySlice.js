  ...
  async (email, thunkAPI) => {
    try {
      const res = await axios.get(
        `${import.meta.env.VITE_API_URL}/family/get/${email}`
      );
      return res.data;
    } catch (error) {
      return thunkAPI.rejectWithValue(
...
export const getParticularFamily = createAsyncThunk(
  ...
  async (familyId, thunkAPI) => {
    try {
      const res = await axios.get(
        `${import.meta.env.VITE_API_URL}/family/get-family/${familyId}`
      );
      return res.data;
    } catch (error) {
      return thunkAPI.rejectWithValue(
  ...
  async ({ familyId, memberId }, thunkAPI) => {
    try {
      const res = await axios.delete(
        ...
        }/family/delete-member/${familyId}/${memberId}`
      );
      return res.data;
    } catch (error) {
      return thunkAPI.rejectWithValue(
  ...
  async ({ familyId, memberId, role }, thunkAPI) => {
    try {
      const res = await axios.put(
        `${
          import.meta.env.VITE_API_URL
        }/family/update-member/${familyId}/${memberId}`,
        { role }
      );
      return res.data;
    } catch (error) {
      return thunkAPI.rejectWithValue(
        error.response?.data?.message ||
          "Failed to update role of family member"
      );
    }
  }
);
...
export const updateFamily = createAsyncThunk(
  ...
  async ({ familyId, formData }, thunkAPI) => {
    try {
      const res = await axios.put(
        `${import.meta.env.VITE_API_URL}/family/update-family/${familyId}`,
        { formData }
      );
      return res.data;
    } catch (error) {
  ...
  async (userId, thunkAPI) => {
    try {
      const res = await axios.get(
        `${
          import.meta.env.VITE_API_URL
        }/family/five-random-suggestions/${userId}`
      );
      return res.data;
    } catch (error) {
      return thunkAPI.rejectWithValue(
...
export const joinRandomFamily = createAsyncThunk(
  ...
  async (data, thunkAPI) => {
    try {
      const res = await axios.post(
        `${import.meta.env.VITE_API_URL}/family/join-random-family`,
        data
      );
      return res.data;
    } catch (error) {
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(updateFamily.fulfilled, (state, action) => {
        ...
        if (state.families) {
          state.families = state.families.map((family) =>
            family._id === action.payload.data._id
              ? action.payload.data
              : family
          );
        }
      ...
      .addCase(get5RandomFamilies.rejected, (state, action) => {
        state.loading = false;
        state.error =
          action.payload || "Something went wrong while fetching suggestions";
      });
  },
});
